...
export const getBlogBySlug = async (req, res) => {
  try {
    ...
    const likeQuery = req.user?.id
      ? { blog: blog._id, user: req.user.id }
      : req.guestToken
        ? { blog: blog._id, guestToken: req.guestToken }
        : null;

    const likedByUser = likeQuery
      ? Boolean(await Like.exists(likeQuery))
      : false;
    ...
    res.json({
      ...
      data: {
        ...
        likesCount: await Like.countDocuments({ blog: blog._id }),
